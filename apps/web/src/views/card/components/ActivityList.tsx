...
const getActivityText = ({
	...
	if (type === 'card.updated.list' && fromList && toList) {
		return (
			<Trans>
				moved the card from{' '}
				<TextHighlight>{truncate(fromList)}</TextHighlight> to
	...
	if (type === 'card.updated.member.added' && displayName) {
		...
		return (
			<Trans>
				assigned <TextHighlight>{truncate(displayName)}</TextHighlight>{' '}
				to the card
			</Trans>
		);
	}
	...
	if (type === 'card.updated.member.removed' && displayName) {
		...
		return (
			<Trans>
				unassigned{' '}
				<TextHighlight>{truncate(displayName)}</TextHighlight> from the
				card
			</Trans>
		);
	}
	...
	if (type === 'card.updated.checklist.item.uncompleted' && toTitle) {
		return (
			<Trans>
				...
				<TextHighlight>{truncate(toTitle)}</TextHighlight> as incomplete
			</Trans>
		);
	}
		...
		<div className="flex flex-col space-y-4 pt-4">
			{allActivities.map((activity, index) => {
				const activityText = getActivityText({
					...
					mergedLabels: (activity as ActivityWithMergedLabels)
						.mergedLabels,
					attachmentName:
