	...
	return db.query.workspaces.findFirst({
		...
		with: {
			members: {
				...
				orderBy: (member, { desc }) => [
					desc(
						sql`CASE WHEN ${member.role} = 'admin' THEN 1 ELSE 0 END`,
					),
...
const parseTicketId = (
	...
	const numberPart = match.at(2);

	if (!prefix || !numberPart) return null;

	return { prefix: prefix.toUpperCase(), number: parseInt(numberPart, 10) };
};
