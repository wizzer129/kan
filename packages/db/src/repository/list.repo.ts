...
export const create = async (
	...
	return db.transaction(async (tx) => {
		const list = await tx.query.lists.findFirst({
			...
			where: and(
				eq(lists.boardId, listInput.boardId),
		...
		if (!result)
			throw new Error(
				`Failed to create list for board ${listInput.boardId}`,
			);
		...
		const duplicateIndices = await tx
			...
			.from(lists)
			.where(
				and(eq(lists.boardId, result.boardId), isNull(lists.deletedAt)),
			)
			.groupBy(lists.index)
      ...
      `);
			...
			const postFixDupes = await tx
				...
				.where(
					and(
						eq(lists.boardId, result.boardId),
						isNull(lists.deletedAt),
					),
				)
				.groupBy(lists.index)
...
> => {
	return db.transaction(async (tx) => {
		...
		if (!list)
			throw new Error(
				`List not found for public ID ${args.listPublicId}`,
			);
    ...
    `);
		...
		const duplicateIndices = await tx
			...
			.from(lists)
			.where(
				and(eq(lists.boardId, list.boardId), isNull(lists.deletedAt)),
			)
			.groupBy(lists.index)
			.having(gt(countExpr, 1));
