			...
			lists: {
				...
				with: {
					cards: {
						...
						where: and(
							cardIds.length > 0
								? inArray(cards.publicId, cardIds)
								: undefined,
							isNull(cards.deletedAt),
...
export const create = async (
	...
	const [result] = await db
		...
		.values({
			...
			backgroundColor: boardInput.backgroundColor ?? null,
			borderColor: boardInput.borderColor ?? null,
		})
		.returning({
			id: boards.id,
...
export const update = async (
	...
	const [result] = await db
		...
		.set({
			...
			...(boardInput.backgroundColor !== undefined && {
				backgroundColor: boardInput.backgroundColor,
			}),
			...(boardInput.borderColor !== undefined && {
				borderColor: boardInput.borderColor,
			}),
		})
...
export const createFromSnapshot = async (
	...
	return db.transaction(async (tx) => {
		...
		const srcLists = [...args.source.lists].sort(
			(a, b) => a.index - b.index,
		);
		if (srcLists.length) {
			...
			for (const list of insertedLists)
				listIndexToId.set(list.index, list.id);
		}
					...
					for (const checklist of sortedChecklists) {
						...
						if (checklist.items.length) {
							...
							if (itemValues.length) {
								...
								const itemActivities = itemValues.map(
									(item) => ({
										publicId: generateUID(),
