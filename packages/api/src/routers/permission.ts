...
export const permissionRouter = createTRPCRouter({
	getMyPermissions: protectedProcedure
		.meta({
			openapi: {
				...
				description:
					"Get the current user's permissions in a workspace",
				tags: ['Permissions'],
				protect: true,
			},
		})
	...
	getMemberPermissions: protectedProcedure
		.meta({
			openapi: {
				...
				description:
					"Get a specific member's permissions in a workspace",
				tags: ['Permissions'],
				protect: true,
			},
		})
	...
	resetMemberPermissions: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			await permissionRepo.clearMemberPermissionOverrides(
				ctx.db,
				member.id,
			);
			...
			const rolesWithPermissions = await Promise.all(
				roles.map(async (role) => {
					const permissionsForRole =
						await permissionRepo.getPermissionsByRoleId(
							ctx.db,
							role.id,
						);
