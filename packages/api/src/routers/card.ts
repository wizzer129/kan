...
export const cardRouter = createTRPCRouter({
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (existingLabel) {
				const deletedCardLabelRelationship =
					await cardRepo.hardDeleteCardLabelRelationship(
						ctx.db,
						cardLabelIds,
					);
			...
			const newCardLabelRelationship =
				await cardRepo.createCardLabelRelationship(
					ctx.db,
					cardLabelIds,
				);
	...
	addOrRemoveMember: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const newCardMemberRelationship =
				await cardRepo.createCardMemberRelationship(
					ctx.db,
					cardMemberIds,
				);
	...
	byId: publicProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			const cardResponse: z.infer<typeof cardDetailSchema> = {
				...result,
				borderColor:
					(result as { borderColor?: string | null }).borderColor ??
					null,
			...
			const activitiesWithAvatarUrls = await Promise.all(
				result.activities.map(async (activity) => {
					...
					if (activity.user?.image) {
						const userAvatarUrl = await generateAvatarUrl(
							activity.user.image,
						);
	...
	update: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const webhookChanges: Record<
				string,
				{ from: unknown; to: unknown }
			> = {};
			if (input.title && existingCard.title !== input.title) {
				webhookChanges.title = {
					from: existingCard.title,
					to: input.title,
				};
			}
			if (
			...
			if (movedToNewList) {
				webhookChanges.listId = {
					...
					to: newList?.publicId ?? existingCard.list.publicId,
				};
			}
	...
	delete: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const fullCard = await cardRepo.getByPublicId(
				ctx.db,
				input.cardPublicId,
			);
	...
	duplicate: protectedProcedure
		.meta({
			openapi: {
				...
				description:
					'Duplicates a card to a target list with optional options',
				tags: ['Cards'],
				protect: true,
			},
		})
		...
		.mutation(async ({ ctx, input }) => {
			...
			const sourceCardMeta =
				await cardRepo.getWorkspaceAndCardIdByCardPublicId(
					ctx.db,
					input.cardPublicId,
				);
			...
			const newCard = await cardRepo.create(ctx.db, {
				...
				borderColor:
					(sourceCard as { borderColor?: string | null })
						.borderColor ?? null,
			});
			...
			if (input.copyMembers && sourceCard.members.length) {
				const memberPublicIds = sourceCard.members.map(
					(m) => m.publicId,
				);
				const members = await workspaceRepo.getAllMembersByPublicIds(
					ctx.db,
