...
export const boardRouter = createTRPCRouter({
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (input.labels.length) {
				const labelInputs = input.labels.map((label, index) => ({
					...
					createdBy: userId,
					colourCode:
						colours[index % colours.length]?.code ?? '#0d9488',
				}));
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (input.favorite !== undefined) {
				if (input.favorite) {
					...
					await boardRepo.removeUserFavorite(
						ctx.db,
						userId,
						board.id,
					);
			...
			if (input.slug) {
				const isBoardSlugAvailable =
					await boardRepo.isBoardSlugAvailable(
						ctx.db,
						input.slug,
						board.workspaceId,
					);
	...
	delete: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (listIds.length) {
				const deletedLists = await listRepo.softDeleteAllByBoardId(
					ctx.db,
					{
						boardId: board.id,
						deletedAt,
				...
				const deletedCards = await cardRepo.softDeleteAllByListIds(
					ctx.db,
					{
						listIds,
						deletedAt,
