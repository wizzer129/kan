		...
		user: {
			create: {
				async before(user: BetterAuthUser, _context: unknown) {
					if (
						env('NEXT_PUBLIC_DISABLE_SIGN_UP')?.toLowerCase() ===
						'true'
					) {
						const pendingInvitation =
							await memberRepo.getByEmailAndStatus(
								db,
								user.email,
								'invited',
							);
					...
					const allowed =
						process.env.BETTER_AUTH_ALLOWED_DOMAINS?.split(',')
						...
						try {
							...
							await client.send(
								new PutObjectCommand({
									Bucket:
										env('NEXT_PUBLIC_AVATAR_BUCKET_NAME') ??
										'',
									Key: key,
									Body: imageBuffer,
					...
					if (notificationClient) {
						try {
							...
							const unsubscribeUrl =
								await createEmailUnsubscribeLink(user.id);
	...
	return {
		after: createAuthMiddleware(async (ctx) => {
			if (
				...
				const callbackURL = ctx.query?.callbackURL as
					| string
					| undefined;
